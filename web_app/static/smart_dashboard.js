...
class SmartDigitalTwin {
    constructor() {
        ...
        this.requestTimeout = 30000; // 30 second timeout for requests
    ...
    renderAICoaching(coaching) {
        ...
        container.innerHTML = `
            <div class="ai-coaching-content">
                ...
                ${coaching.coaching && Array.isArray(coaching.coaching) ? coaching.coaching.map(insight => `
                    <div class="coaching-insight mb-3 p-3 border rounded" style="border-left: 4px solid ${this.getInsightColor(insight.priority || 'medium')};">
                        <div class="insight-header d-flex justify-content-between align-items-start mb-2">
                            <h6 class="mb-0 text-dark">${insight.title || 'AI Insight'}</h6>
                            <span class="badge badge-${this.getPriorityBadgeClass(insight.priority || 'medium')}">${insight.priority || 'medium'}</span>
                        </div>
                        <p class="mb-2 text-secondary">${insight.message || 'No message available'}</p>
                        ...
                        ${insight.recommendations && Array.isArray(insight.recommendations) && insight.recommendations.length > 0 ? `
                            <div class="recommendations">
                                ...
                                <ul class="small mt-1 mb-0">
                                    ${insight.recommendations.map(rec => `<li>${rec || 'No recommendation'}</li>`).join('')}
                                </ul>
                            </div>
                        ` : ''}
                        
                        <div class="insight-meta mt-2 d-flex justify-content-between align-items-center">
                            <small class="text-muted">Confidence: ${((insight.confidence || 0.5) * 100).toFixed(0)}%</small>
                            <small class="text-muted">${insight.type ? insight.type.replace('_', ' ') : 'General'}</small>
                        </div>
                    </div>
                `).join('') : '<p class="text-muted">No coaching insights available yet.</p>'}
    ...
    async loadBehavioralAnalytics() {
        ...
        try {
            const response = await this.fetchWithTimeout('/behavioral-insights/Paresh', {}, 2000); // Optimized timeout for fast response
            ...
            container.innerHTML = `
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">Loading behavioral analytics...</p>
                    <small class="text-muted">
                        Install the Chrome extension and start using the system to see your real behavioral data.
                    </small>
                    <div class="mt-3">
                        <button class="btn btn-outline-primary btn-sm" onclick="window.twin.loadBehavioralAnalytics()">
    ...
    async loadCollaborationSuggestions() {
        try {
            ...
            if (response.ok) {
                ...
                this.updateSuggestionsCount(data.count || 0);
            }
        } catch (error) {
    ...
    async loadCollaborationAnalytics() {
        try {
            ...
            if (response.ok) {
                ...
                this.updateQuickStats(analytics.summary);
            }
        } catch (error) {
    ...
    renderPlatformStatus(status) {
        ...
        if (slackStatus) {
            slackStatus.className = status.slack.connected ? 'badge bg-success' : 'badge bg-danger';
        ...
        if (teamsStatus) {
            teamsStatus.className = status.teams.connected ? 'badge bg-success' : 'badge bg-danger';
    ...
    renderCollaborationSuggestions(suggestions) {
        ...
        const suggestionsHtml = suggestions.map(suggestion => {
            const confidenceColor = suggestion.confidence >= 0.8 ? 'success' : 
                                   suggestion.confidence >= 0.6 ? 'warning' : 'secondary';
            const typeIcon = {
                'create_task': 'fas fa-tasks',
                'schedule_meeting': 'fas fa-calendar-plus',
                'send_reminder': 'fas fa-bell',
                'auto_reply': 'fas fa-reply'
            }[suggestion.type] || 'fas fa-lightbulb';
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <small class="text-muted">
                                        ${suggestion.context.platform} • ${suggestion.context.channel}
                                    </small>
                                </div>
                            </div>
                            <div class="d-flex gap-1 ms-3">
                                <button class="btn btn-success btn-sm" onclick="window.twin.executeSuggestion('${suggestion.id}', true)">
    ...
    renderRecentMessages(messages) {
        ...
        const messagesHtml = messages.slice(0, 5).map(msg => {
            ...
            const priorityColor = analysis && analysis.priority_score >= 8 ? 'danger' : 
                                 analysis && analysis.priority_score >= 6 ? 'warning' : 'secondary';
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            <div class="d-flex align-items-center mb-1">
                                <i class="${platformIcon} text-muted me-2"></i>
                                <strong class="text-primary">${message.user}</strong>
                                <span class="text-muted mx-2">•</span>
                                <small class="text-muted">${message.channel}</small>
                            </div>
                            <p class="mb-1 small">${message.text.substring(0, 100)}${message.text.length > 100 ? '...' : ''}</p>
                            ${analysis && analysis.action_items.length > 0 ? 
                                `<div class="mt-1">
                                    <span class="badge bg-info me-1">${analysis.action_items.length} actions</span>
                                    ${analysis.deadlines.length > 0 ? `<span class="badge bg-warning">${analysis.deadlines.length} deadlines</span>` : ''}
                                </div>` : ''}
                        </div>
                        <div class="text-end">
                            <small class="text-muted">${new Date(message.timestamp).toLocaleTimeString()}</small>
                            ${analysis ? `<br><span class="badge bg-${priorityColor}">${analysis.priority_score.toFixed(1)}</span>` : ''}
                        </div>
                    </div>
                </div>
            `;
            ...
            <div class="row g-3">
                <div class="col-6">
                    <div class="text-center">
                        <div class="h4 text-primary mb-0">${summary.total_messages || 0}</div>
                        <small class="text-muted">Messages Analyzed</small>
                    </div>
                </div>
                <div class="col-6">
                    <div class="text-center">
                        <div class="h4 text-success mb-0">${summary.actions_executed || 0}</div>
                        <small class="text-muted">Actions Executed</small>
                    </div>
                </div>
                <div class="col-12">
                    <div class="mt-2">
                        <small class="text-muted d-block">Platform Distribution:</small>
                        ${Object.entries(breakdown.by_platform || {}).map(([platform, count]) => 
                            `<span class="badge bg-secondary me-1">${platform}: ${count}</span>`
                        ).join('')}
                    </div>
                </div>
            </div>
    ...
    updateQuickStats(summary) {
        const elements = {
            ...
            'pendingSuggestions': summary.suggestions_generated || 0,
            'actionItems': summary.action_items_identified || 0
        };
    ...
    populateConfigForm(config) {
        ...
        if (autoCreateTasks) {
            autoCreateTasks.checked = config.auto_actions_enabled;
        }
    }
    ...
    async executeSuggestion(suggestionId, approved) {
        try {
            const response = await this.fetchWithTimeout(
                `${this.collaborationApiUrl}/suggestions/${suggestionId}/execute`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action_id: suggestionId, approved })
                }
            );
            ...
            if (response.ok) {
                const result = await response.json();
                if (approved) {
                    this.showNotification('success', `Action executed: ${result.action.title}`);
                } else {
                    this.showNotification('info', 'Action dismissed');
                }
            ...
            this.showNotification('error', 'Failed to execute action');
        }
    }
    ...
    async saveCollaborationConfig() {
        try {
            ...
            const autoActions = document.getElementById('autoCreateTasks').checked;

            const config = {
                channels: selectedChannels,
                users: usersInput.split(',').map(u => u.trim()).filter(u => u),
                keywords: keywordsInput.split(',').map(k => k.trim()).filter(k => k),
                auto_actions_enabled: autoActions,
                platforms: ['slack', 'teams']
            };

            const response = await this.fetchWithTimeout(
                `${this.collaborationApiUrl}/monitoring/config`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    toggleCollaborationConfig() {
        const configPanel = document.getElementById('collaborationConfig');
        // Toggle edit mode styling or show/hide advanced options
        console.log('Toggle collaboration config');
    }
            ...
            if (container) {
                container.innerHTML = `
                    <div class="text-center py-4">
                        ...
                        <p class="text-muted">Collaboration API unavailable</p>
                        <small class="text-muted">Start the collaboration server on port 8001</small>
                    </div>
                `;
            }
        });
    ...
    startCollaborationPolling() {
        ...
        setInterval(async () => {
            try {
                ...
                console.warn('Collaboration polling update failed:', error);
            }
        }, 30000);
    }
}
